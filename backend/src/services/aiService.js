...
async function testAuxiliaryFromConfigRow(configRow) {
  const extra = typeof configRow?.extra === 'string' ? JSON.parse(configRow.extra || '{}') : (configRow?.extra || {});
  if (!extra.aux_enabled) throw new Error('辅助LLM未启用');
  const auxConfig = {
    provider_type: extra.aux_provider_type || 'openai-compatible',
    base_url: extra.aux_base_url || '',
    api_key: extra.aux_api_key || '',
    model: extra.aux_model || '',
    system_prompt: extra.aux_system_prompt || '',
  };
  if (!auxConfig.api_key || !auxConfig.model) {
    throw new Error('辅助LLM缺少 api_key 或 model');
  }
  if (auxConfig.provider_type === 'openai-compatible' && !auxConfig.base_url) {
    throw new Error('辅助LLM缺少 base_url');
  }
  const handler = PROVIDER_HANDLERS[auxConfig.provider_type];
  if (!handler) throw new Error(`辅助LLM provider 不支持: ${auxConfig.provider_type}`);
  const text = await handler(auxConfig, '请回复"辅助连接成功"五个字，不要其他内容。', 30);
  return text;
}
